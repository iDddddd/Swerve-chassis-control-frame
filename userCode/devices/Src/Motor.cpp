...
void Motor::CANPackageSend() {
    ...
    for(auto motorIndex = 0; motorIndex < 16; motorIndex++) {
        if((1 << motorIndex) & motor_IDs){
            txHeaderTypeDef.StdId = 0x141 + motorIndex;
            canBuf[0] = 0xA1;
            canBuf[4] = motor_intensity[motorIndex];
            canBuf[5] = motor_intensity[motorIndex] >> 8u;
					if (motorIndex<2)
            HAL_CAN_AddTxMessage(&hcan1,&txHeaderTypeDef,canBuf,(uint32_t *) CAN_TX_MAILBOX0);
					else HAL_CAN_AddTxMessage(&hcan1,&txHeaderTypeDef,canBuf,(uint32_t *) CAN_TX_MAILBOX1);
        }
    }
...
void Motor::IT_Handle(CAN_HandleTypeDef *hcan) {
    ...
    uint8_t canBuftest[8];
    CAN_RxHeaderTypeDef rx_header;
    HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &rx_header, canBuftest);
    ...
    motorPtrs[canPos][motorPos]->feedback.angle = (canBuftest[7]<<8u)|canBuftest[6];
		int16_t test = (canBuftest[7]<<8u)|canBuftest[6];
    motorPtrs[canPos][motorPos]->feedback.speed = (canBuftest[5]<<8u)|canBuftest[4];
    motorPtrs[canPos][motorPos]->feedback.moment = (canBuftest[3]<<8u)|canBuftest[2];
    motorPtrs[canPos][motorPos]->feedback.temp = canBuftest[1];
...
void Motor::MotorStateUpdate() {
    ...
    if (thisAngle <= lastRead) {
        if (lastRead - thisAngle > 8000)
            realAngle += (thisAngle + 16383 - lastRead) * 360.0f / 16383.0f / reductionRatio;
        else
            realAngle -= (lastRead - thisAngle) * 360.0f / 16383.0f / reductionRatio;
    } else {
        if (thisAngle - lastRead > 8000)
            realAngle -= (lastRead + 16383 - thisAngle) * 360.0f / 16383.0f / reductionRatio;
        else
            realAngle += (thisAngle - lastRead) * 360.0f / 16383.0f / reductionRatio;
    }
